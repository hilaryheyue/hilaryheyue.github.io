...
function setup() {
  ...
  for (i = 0; i < hour() % 12; i++) {
    startX = 100 + i * 50;
    startY = 100;
    this.p = new ParticleSystem(createVector(startX, startY));
    systems.push(p);
  }
}
...
function draw() {
  ...
  fill(255);
  textSize(24);
  text("# of clusters = hour, pointer angle = minute", 50, 50);
}
...
function CrazyParticle(origin) {
  ...
  this.theta = map(minute() + norm(second(), 0, 60), 0, 60, 0, TWO_PI) - HALF_PI;;
};
