...
function draw() {
    var mouseText = false; // prevent two texts showing at the same time
    background(255);
    noStroke();
    fill(0);
    textSize(24);
    ...
    textSize (17);
    fill(150);
    text('click dots to see year and price info', offset, 58);
    ...
    for (var t = 0; t <= t_max; t++) {
        for (var k = 0; k < lineX.length; k++) {
            ...
            if (t % 12 == 0) {
                ...
                if (abs(mouseX - lineX[k][t]) < 5 && abs(mouseY - lineY[k][t]) < 5 && !mouseText) {
                    textSize(17);
                    noStroke();
                    fill(0);
                    var year = 1995 + t / 12;
                    text(year + ',' + '$' + round((indexHeight - lineY[k][t]) * 100) / 100, mouseX, mouseY + 12);
                    mouseText = true;
                }
            }
        }
    ...
    var mx = mouseX + lengthPerDollar * 4;
    var my = mouseY - (indexHeight + 40) - (topMargin + lengthPerDollar * 4);
    for (var k = 0; k < x.length; k++) {
        if (k % 2 == 0) {
            // draw circles
            translate(offset + lengthPerDollar * 8, 0);
            mx -= offset + lengthPerDollar * 8;
            for (var i = 1; i <= 4; i++) {
                noFill();
                stroke(50, 25);
                strokeWeight(1);
            ...
            textSize(17);
            fill(0);
            text(labels[k / 2], -textWidth(labels[k / 2]) / 2, lengthPerDollar * 4 + 40);
        }
        for (var t = 0; t <= t_max; t++) {
            if (t <= nt / 2) {
                var inter = map(t, 0, nt / 2, 0, 1);
            ...
            strokeWeight(4);
            point(x[k][t], y[k][t]);
            if (abs(mx - x[k][t]) < 5 && abs(my - y[k][t]) < 5 && !mouseText) {
                textSize(17);
                noStroke();
                fill(0);
                var r = sqrt(sq(x[k][t]) + sq(y[k][t])) / lengthPerDollar;
                var year = floor(1995 + t / 12);
                text(year + ',' + '$' + round(r * 100) / 100, mx, my);17
                mouseText = true;
            }
        }
    }
