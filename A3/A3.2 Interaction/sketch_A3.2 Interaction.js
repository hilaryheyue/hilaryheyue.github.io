...
function draw() {
  ...
  for (var i = 0; i < headlines.length; i++) {
    ...
    for (var j=0; j<words.length; j++) {
      // draw headline
      var d = textWidth(words[j]+' ');
      noStroke()
      fill(color(200, 200, 0, 50));
      ellipse(nextX + d / 2, i * lineheight, d, d);

      if (mouseX - margin > nextX 
      	  && mouseX - margin < nextX + d
      	  && mouseY - margin > (i-1) * lineheight 
